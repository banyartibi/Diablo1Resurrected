...
### Mentás-fagyás hardolása — Drain lock nélkül (`render_bridge.cpp`):
* **Hiba:** `Esc → Save Game` a Godot motoron még a sorbaűzési javítás után is kifagyhatott. A `PollGodotBridgeInput()` dréelő switch blokk TARTJA a `g_BridgeJobMutex`-et minden feladat végrehajtása közben. Egy hosszú művelet (Mentás → `ActivateGamemenuItem` → `gamemenu_save_game` → `DrawAndBlit`/`interface_msg_pump`) ilyenkor blokkolta a mutexet, ami re-entráns `PushBridgeAction()`-ra deadlokot/fagyást okozhatott, és az `g_BridgeJobs.clear()` zártan volt race-elve.
* **Javítás:** a munkasort most már **pillanatilag lefűzzük + kiürítjük A mutex tartása alatt** (`drainedJobs.swap(g_BridgeJobs)`), majd a feladatokat **a mutex felszabadítása után** hajtjuk végre. Így a hosszú műveletek már nem tartják a mutexet → megszűnik a re-entráns deadlock/fagyás kockája, és eltűnik az `clear()` race-e.
* **Ellenőrzés:** `ninja` a core-ra és a GDExtension-re is **0 hibával** fut (csak meglévő fmt/-Wtype-limits/-Walloc-size figyelmeztetések); az `.so` újrafordítva.

### Single-executable-on-Godot politika (`run_d1_godot3d.sh`, `run_devilutionx_hd.sh`):
* **Hiba:** A háttérben futó standalone `DevilutionX/build/devilutionx` egy **második executable**-ként jelenhetett meg a Godot GDExtension mellett — pedig a GDExtension már leváltotta, és mindig **egy executable**-nek, mindig **a godot motoron** kell futnia, akkor is, ha a legacy/original-2.5D megjelenítés megy.
* **Javítás:**
  1. `run_d1_godot3d.sh` most indítás előtt **megöli az összes maradt standalone DevilolutionX folyamatot** (`pgrep -f .../DevilutionX/build/devilutionx`), így sosem marad kettő, és **ellenőrzi a `.so` meglétét**, mielőtt elindítaná a motort.
