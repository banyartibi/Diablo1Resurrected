...
AssetRef FindAsset(const char *filename)
{
	...
	// 1. Direct workspace assets folder:
	{
		const std::string wsPath = "/home/biti/antigravity/magical-bell/assets/" + relativePath;
		result.directHandle = OpenOptionalRWops(wsPath);
		if (result.directHandle != nullptr) {
			Log("Loaded HD Asset Override from workspace: {}", wsPath);
			return result;
		}
	}

	// HD Asset Override: Check the `/assets` directory next to the devilutionx binary first:
	result.directHandle = OpenOptionalRWops(paths::AssetsPath() + relativePath);
	if (result.directHandle != nullptr) {
		Log("Loaded HD Asset Override from assets: {}", paths::AssetsPath() + relativePath);
		return result;
	}
