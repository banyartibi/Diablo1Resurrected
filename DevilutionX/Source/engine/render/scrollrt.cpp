...
ZoomMode CurrentZoomMode = ZoomMode::Zoomed_2x;
 ...
 */
void Zoom1_5x(const Surface &out)
{
	int vw = out.w();
	int vh = out.h();
	if (vw <= 0 || vh <= 0)
...
void DrawGame(const Surface &fullOut, Point position, Displacement offset)
{
	...
	const Surface &out = (CurrentZoomMode == ZoomMode::Normal)
	...
	// Skip rendering parts covered by the panels
	if (CanPanelsCoverView() && (IsLeftPanelOpen() || IsRightPanelOpen())) {
		columns -= (CurrentZoomMode != ZoomMode::Normal) ? 2 : 4;
	}
...
void DrawFPS(const Surface &out)
{
	...
	case ShaderStyle::AI_Neural_CNN:
		...
		break;
	case ShaderStyle::AI_Neural_Bloom:
		DrawString(out, "Style [2/12]: [AI Neural CNN + Volumetric Fire Glow & Embers]", Point { 8, 86 }, { UiFlags::ColorGold });
		break;
	case ShaderStyle::xBRZ_Vector:
		DrawString(out, "Style [3/12]: [xBRZ 2D Vector Reconstruction (Hyllian)]", Point { 8, 86 }, { UiFlags::ColorGold });
		break;
	case ShaderStyle::xBRZ_Bloom:
		DrawString(out, "Style [4/12]: [xBRZ Vector + Volumetric Fire Glow & Embers]", Point { 8, 86 }, { UiFlags::ColorGold });
		break;
	case ShaderStyle::Anime4K_Line:
		DrawString(out, "Style [5/12]: [Anime4K 2D Contour & Edge Refinement]", Point { 8, 86 }, { UiFlags::ColorGold });
		break;
	case ShaderStyle::Scale3X_SABR:
		DrawString(out, "Style [6/12]: [Scale3x / ScaleFX Curved Boundary]", Point { 8, 86 }, { UiFlags::ColorGold });
		break;
	case ShaderStyle::FSR_Ultra_Smooth:
		DrawString(out, "Style [7/12]: [AMD FSR Ultra Smooth (EASU + Volumetric Fire & Embers)]", Point { 8, 86 }, { UiFlags::ColorGold });
		break;
	case ShaderStyle::FSR_CMAA2_Clean:
		DrawString(out, "Style [8/12]: [AMD FSR + Intel CMAA2 Pro (100% Anti-Aliased)]", Point { 8, 86 }, { UiFlags::ColorGold });
		break;
	case ShaderStyle::Bilateral_Denoise:
		DrawString(out, "Style [9/12]: [Bilateral Denoised HD (Retro Dither Removal)]", Point { 8, 86 }, { UiFlags::ColorGold });
		break;
	case ShaderStyle::Dark_CRT_Royale:
		DrawString(out, "Style [10/12]: [Dark-CRT Royale (Curved Glass + Scanlines)]", Point { 8, 86 }, { UiFlags::ColorGold });
		break;
	case ShaderStyle::Flat_CRT_Trinitron:
		DrawString(out, "Style [11/12]: [Flat CRT Trinitron (Sharp Aperture Grille)]", Point { 8, 86 }, { UiFlags::ColorGold });
		break;
	case ShaderStyle::Vanilla_PixelArt:
	default:
		DrawString(out, "Style [12/12]: [Integer Scaling 1:1 (Vanilla Pixel-Art)]", Point { 8, 86 }, { UiFlags::ColorWhite });
		break;
	}

	if (Vulkan_IsActive()) {
		DrawString(out, "Backend: [Vulkan] | F9: AI Generat\xc3\xadv | F10: Sz\xc3\xadnprofil | F11/F12: Shader", Point { 8, 104 }, { UiFlags::ColorGold });
	}
	DrawString(out, "Build: v3.0.0-HD (AMD RX 6800 XT)", Point { 8, 122 }, { UiFlags::ColorWhite });
	if (CurrentZoomMode == ZoomMode::Zoomed_2x) {
		DrawString(out, "Zoom: [2.0x (K\xc3\xb6zeli / Zoomed)]", Point { 8, 140 }, { UiFlags::ColorGold });
	} else if (CurrentZoomMode == ZoomMode::Balanced_1_5x) {
		DrawString(out, "Zoom: [1.5x (K\xc3\xb6ztes / Arany K\xc3\xb6z\xc3\xa9p\xc3\xbat)]", Point { 8, 140 }, { UiFlags::ColorGold });
	} else {
		DrawString(out, "Zoom: [1.0x (Norm\xc3\xa1l / Sz\xc3\xa9les)]", Point { 8, 140 }, { UiFlags::ColorWhite });
	}

	switch (CurrentAtmosphereFx) {
	case AtmosphereFxMode::All_On:
		DrawString(out, "Atmosphere [F9]: [1/4: ALL ON (SSAO + Wetness + Hero Light + Mist)]", Point { 8, 158 }, { UiFlags::ColorGold });
		break;
	case AtmosphereFxMode::Lights_And_Mist:
		DrawString(out, "Atmosphere [F9]: [2/4: LIGHTS & MIST (Hero Candle + Mist + Torches)]", Point { 8, 158 }, { UiFlags::ColorGold });
		break;
	case AtmosphereFxMode::Shadows_And_Wetness:
		DrawString(out, "Atmosphere [F9]: [3/4: SHADOWS & WETNESS (SSAO + Specular Wetness)]", Point { 8, 158 }, { UiFlags::ColorGold });
		break;
	case AtmosphereFxMode::Off:
	default:
		DrawString(out, "Atmosphere [F9]: [4/4: OFF (Vanilla 2D Direct Render)]", Point { 8, 158 }, { UiFlags::ColorWhite });
		break;
	}

	switch (CurrentColorProfile) {
	case ColorProfile::D4_Dark_Gothic:
		DrawString(out, "Color [F10]: [1/5: Dark Gothic (OLED Black & Warm Embers)]", Point { 8, 176 }, { UiFlags::ColorGold });
		break;
	case ColorProfile::Hellish_Crimson:
		DrawString(out, "Color [F10]: [2/5: Hellish Crimson (Warm Gothic & Subtle Red)]", Point { 8, 176 }, { UiFlags::ColorRed });
		break;
	case ColorProfile::High_Contrast_Vibrant:
		DrawString(out, "Color [F10]: [3/5: High-Contrast Vibrant (32-bit Saturated Glow)]", Point { 8, 176 }, { UiFlags::ColorGold });
		break;
	case ColorProfile::Cold_Crypt:
		DrawString(out, "Color [F10]: [4/5: Cold Crypt (Blue-Gray Gothic Shadow)]", Point { 8, 176 }, { UiFlags::ColorBlue });
		break;
	case ColorProfile::Neutral_1996_Classic:
	default:
		DrawString(out, "Color [F10]: [5/5: 1996 Classic (32-bit De-Banded)]", Point { 8, 176 }, { UiFlags::ColorWhite });
		break;
	}
}
...
void CalcViewportGeometry()
{
	...
	const int screenHeight = (GetScreenHeight() * multiplier) / divisor;
	const int panelHeight = (GetMainPanel().size.height * multiplier) / divisor;
	...
	const int viewportHeight = (GetViewportHeight() * multiplier) / divisor;
