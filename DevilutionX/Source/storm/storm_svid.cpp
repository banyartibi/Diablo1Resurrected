...
bool BlitFrame()
{
	...
	SDL_Rect outputRect;
	if (IsLandscapeFit(SVidWidth, SVidHeight, outputSurface->w, outputSurface->h)) {
		outputRect.w = outputSurface->w;
		outputRect.h = static_cast<int>(SVidHeight * outputSurface->w / SVidWidth);
	} else {
		outputRect.w = static_cast<int>(SVidWidth * outputSurface->h / SVidHeight);
		outputRect.h = outputSurface->h;
	}
	outputRect.x = (outputSurface->w - outputRect.w) / 2;
	outputRect.y = (outputSurface->h - outputRect.h) / 2;

	SDL_FillRect(outputSurface, nullptr, 0x000000);

	if (SDL_BlitScaled(SVidSurface.get(), nullptr, outputSurface, &outputRect) <= -1) {
		SDLSurfaceUniquePtr converted = SDLWrap::ConvertSurfaceFormat(SVidSurface.get(), SDL_PIXELFORMAT_RGB888, 0);
		if (converted && SDL_BlitScaled(converted.get(), nullptr, outputSurface, &outputRect) > -1) {
			return true;
		}
