...
bool SpawnWindow(const char *lpWindowName)
{
	...
	if (gbVulkanRequested) {
		flags |= SDL_WINDOW_VULKAN;
	}
	int winX = SDL_WINDOWPOS_UNDEFINED;
	int winY = SDL_WINDOWPOS_UNDEFINED;
	int winW = windowSize.width;
	int winH = windowSize.height;
	...
	ghMainWnd = SDL_CreateWindow(lpWindowName, winX, winY, winW, winH, flags);

	if (ghMainWnd != nullptr && std::getenv("D1_MINIMIZE_WINDOW") != nullptr) {
		SDL_SetWindowPosition(ghMainWnd, -4000, -4000);
		SDL_MinimizeWindow(ghMainWnd);
	}
...
#ifndef USE_SDL1
	SDL_DisplayMode mode {};
	if (SDL_GetDisplayMode(0, 0, &mode) == 0 && mode.refresh_rate > 0) {
...
void ReinitializeTexture()
{
	...
	if (texture && *sgOptions.Graphics.scaleQuality == ScalingQuality::NearestPixel) {
...
void ReinitializeRenderer()
{
	...
	if (gbVulkanRequested) {
		if (Vulkan_Init(ghMainWnd, gnScreenWidth, gnScreenHeight)) {
			RendererTextureSurface = SDLWrap::CreateRGBSurfaceWithFormat(0, gnScreenWidth, gnScreenHeight, 32, SDL_PIXELFORMAT_ARGB8888);
			return;
		}
		Log("Vulkan initialization failed, falling back to OpenGL");
	}
...
SDL_Surface *GetOutputSurface()
{
	...
	return ret;
#else
	if (RendererTextureSurface != nullptr)
